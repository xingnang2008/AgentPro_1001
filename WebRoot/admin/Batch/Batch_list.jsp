...
<html>
	...
	$(function(){
		$('#btnSearch').click(function(){
			$('#dg').datagrid('load',{
				sendNo: $('#sendNo').combobox('getValue'),			
		...
		$('#dg').datagrid({    
			   ...
			   loadMsg:'请等待...',
				...
				toolbar: [{
					...
					handler: function(){
						$("#win").window({
							...
							width:'650',
							height:'80%',
							content:'<iframe src="Batch_saveInput.jsp" frameborder="0" width="100%" height="100%"> </iframe>'
						});
					...
					},
					{
						...
						if(rows.length !=1){
							...
							$("#win").window({
								...
								width:'80%',
								height:'90%',
								content:'<iframe title="" src="Batch_updateInput.jsp" frameborder="0" width="100%" height="100%"/>'
							});
					}}},
					{
						iconCls: 'icon-remove',
						...
						if(rows.length ==0){
							...
							$.messager.confirm("删除确认对话框","是否要删除选中的记录",function(r){
								if(r){
									...
									//发送ajax请求
									$.post("Batch/Batch-deleteByIds.action",{ids:ids},function(result){
										if(result =="true"){
			    ...
			    columns:[[ 
					        ...
					        {field:'sendDate',title:'发出日期', 
						        formatter:function(value,row,index){  
		                        	var unixTimestamp = new Date(value);  
		                        	return unixTimestamp.toLocaleDateString();  
		                        } , align:'center',width:80},
					   		        
					        {field:'sendNo',title:'批次',align:'left',width:150},
					        {field:'weight',title:'重量',align:'left',width:100},					        		                   
					        {field:'remarks',title:'备注',align:'right',width:100}
				]]    
			});	
		//客户选取
		$("#sendNo").combobox({
			url:'<%=basePath%>admin/Batch/Batch-listAll.action',
			editable:false,
			valueField:'sendNo',
			textField:'sendNo',
			panelHeight:200,
...
<div id="container"> 
	<div id="searchDiv">
			<div>
			<div id="label">批次</div>
			<div id="hang"><input type="text"  id="sendNo"  name="sendNo"  /></div>		
